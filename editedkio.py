...
class Kio(object):
    ...
    def __init__(self, conversation, agent, slackClient=None):
        ...
        self.messageHistory = []   
        # self.optimalResponse = self.agent.tell_kio     
        self.agent = agent
    ...
    def generateResponse(self, message):
        ...
        if (startup == True):
            response = "Sorry I just woke up. Can you give me 20 seconds?"
            self.slackClient.api_call(
            "chat.postMessage",
            channel=self.conversation.id,
            text=response
            ...
            time.sleep(30)
            startup = False
        # elif (len(Responses) > 1 and ):
        #     response = "Can you hold on for 20 seconds. I'm thinking"
